...
@inline function make_tracer(
    seen::IdDict, prev::RT, path::Tuple, mode::TraceMode; toscalar=false, tobatch=nothing
) where {RT}
    if haskey(seen, prev)
        return seen[prev]
    end
...
@inline function make_tracer(
    ...
    return (
        (
            make_tracer(seen, v, append_path(path, i), mode; toscalar, tobatch) for
...
@inline function make_tracer(
    ...
    return NamedTuple{A,traced_type(RT, (), Val(mode))}((
        (
            make_tracer(
                seen, Base.getfield(prev, i), append_path(path, i), mode; toscalar, tobatch
            ) for i in 1:length(A)
        )...,
    ))
end
